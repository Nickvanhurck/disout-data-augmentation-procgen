...
from common.model import NatureModel, ImpalaModel, DisoutModel
from common.policy import CategoricalPolicy
from common import set_global_seeds, set_global_log_levels
from agents.ppo_aug import PPO_aug as AGENT
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='disout_data_aug', help='experiment name')
    parser.add_argument('--env_name', type=str, default='bigfish', help='environment ID')
    parser.add_argument('--start_level', type=int, default=int(0), help='start-level for environment')
    parser.add_argument('--num_levels', type=int, default=int(200), help='number of training levels for environment')
    ...
    # Model architecture: impala disout
    model = DisoutModel(in_channels=in_channels)
    # print(model)
    ...
    batch_size = hyperparameters.get('mini_batch_size', 2048)

    aug_type = 'crop'

    aug_id = data_augs.Identity
    aug_func = aug_to_func[aug_type](batch_size=batch_size)

    agent = AGENT(env, policy, logger, storage, device, num_checkpoints, aug_id, aug_func, **hyperparameters)
