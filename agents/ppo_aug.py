...
class PPO_aug(BaseAgent):
    def __init__(self,
        ...
        super(PPO_aug, self).__init__(env, policy, logger, storage, device, n_checkpoints)
    ...
    def optimize(self):
        ...
        for e in range(self.epoch):
            ...
            for sample in generator:
                ...
                # added for aug
                _, new_actions_batch, _, _ = self.policy.act(obs_batch_id, hidden_state_batch, mask_batch)
                values_aug, action_log_probs_aug, dist_entropy_aug, _ = self.policy.evaluate_actions(obs_batch_aug,hidden_state_batch, mask_batch, new_actions_batch)

                # Compute Augmented Loss
                action_loss_aug = - action_log_probs_aug.mean()
                value_loss_aug = .5 * (torch.detach(value_batch) - values_aug).pow(2).mean()

                # Update actor-critic using both PPO and Augmented Loss
                # self.optimizer.zero_grad()  # happens below
                aug_loss = value_loss_aug + action_loss_aug

                (value_loss * self.value_coef + pi_loss - entropy_loss * self.entropy_coef + aug_loss * self.aug_coef).backward()
                # torch.nn.utils.clip_grad_norm_(self.policy.parameters(), self.grad_clip_norm)
                # self.optimizer.step()

                # Let model to handle the large batch-size with small gpu-memory (OLD)
