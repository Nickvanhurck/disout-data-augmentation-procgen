...
from .disout import LinearScheduler, Disout
...
class ImpalaBlock_disout(nn.Module):
  def __init__(self, in_channels, out_channels, dist_prob=0.05, nr_steps=256):
    super(ImpalaBlock_disout, self).__init__()
    self.conv = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, stride=1, padding=1)
    
    # added
    self.disout = LinearScheduler(Disout(dist_prob=dist_prob),
...
class DisoutModel(nn.Module):
  def __init__(self,
               in_channels,
               **kwargs):
    super(DisoutModel, self).__init__()
    self.block1 = ImpalaBlock(in_channels=in_channels, out_channels=16)
    self.block2 = ImpalaBlock(in_channels=16, out_channels=32)
    self.block3 = ImpalaBlock_disout(in_channels=32, out_channels=32)
